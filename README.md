...
pkill -f cc_ws_gateway; \
sudo -u erik tmux kill-server 2>/dev/null; sleep 1; \
nohup python3 cc_ws_gateway.py >> logs/cc_gateway.log 2>&1 &
```

网关启动时自动：清理旧 MCP 配置（如有残留的 `erik_channel`）→ 启动 tmux session → CC CLI 加载 CLAUDE.md + MCP 配置 → TranscriptTailer 开始轮询 transcript JSONL。
...
### Transcript Tailing（回复 + thinking + 工具调用）
...
- `{"type": "thinking", "thinking": "...", "signature": "..."}` — 思考块，推 `stream:thinking`
...
注意：Sonnet 4-6 的 thinking 是加密签名的（`thinking: ""` + `signature`），只能显示"正在思考"指示器，看不到实际内容。工具调用完整可见。
...
- **transcript tailing**：实时捕获 text、thinking、tool_use 推前端
...
- 回复为整块推送，非 token 级流式
- CC CLI 重定向 thinking 后迅速擦除内容（仅留签名），tailer 可能来不及读取
