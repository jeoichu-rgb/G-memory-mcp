...
日记按约定（`docs/diary_convention.md`）以【节点】结构书写：梗概、细节挂节点、感受挂节点或【整体】、独白。`write_diary` 落盘后，palace 侧后台线程调 DeepSeek 按【】节点**机械切分**（原文拼接，不概括不改写）：每个事件节点切成一条动态记忆，前缀日期时间，标签从固定集合选（`DIARY_SPLIT_CATEGORIES`），metadata 带 `source: diary_split` 和 `diary_file` 回链。【整体】感受和独白不入库，留在日记里。切出的条目在前端动态面板照常可改可删。没有【】标记的日记跳过切分。
...
| cmd | 说明 | 主要参数 |
|--------|------|---------|
| `write_diary` | 写新日记 MD 文件到 VPS；含【】节点标记时后台自动按节点切进动态库（见"日记节点切分"） | `title`, `content`, `mood`（可选） |
