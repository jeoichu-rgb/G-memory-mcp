...
│                                                      │
│  ┌─ 三层后台 worker（asyncio task，独立于 WS）──────┐  │
│  │  L0 Keepalive  50min 无对话 → 1 token 保活cache  │  │
...
- **触发条件**：距上次缓存刷新 ≥ 50 分钟
- **行为**：向 CC CLI 发送 1 token 消息（`--resume` 同 session），刷新 Anthropic prompt cache TTL
- **目的**：prompt cache 5 分钟 TTL，但保活间隔设为 50 分钟是因为 cache 一旦建立后续对话自动续期，保活只是兜底
...
- Pebbling 开关（🔋）：控制三层后台系统（缓存保活 + 巡查 + 自由活动），状态持久化到 `pebbling_state.json`
...
```bash
cd /opt/G-memory-mcp && git pull && kill $(lsof -t -i :3000) 2>/dev/null; nohup python3 cc_ws_gateway.py > /dev/null 2>&1 &
```
