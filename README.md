...
### 亲密备忘固定注入（Pinned Memos）

📎 的模糊检索按打分返回记忆，亲密对话时可能注入不相关的条目、漏掉真正需要的描写备忘。固定注入把三条描写备忘（床边描写备忘、修辞规则、进程过渡+词汇）**钉死**：触发词命中就整块原文注入，不检索、不打分、不掺别的。
...
{
  "triggers": ["做爱", "爸爸", "..."],
  "ids": ["claude_床边_2.1描写备忘1_修辞规则.md", "..."]
}
```

**行为规则：**

- 📎 开启 + 消息命中任一触发词（大小写不敏感）→ 网关调 `/admin/memories_by_ids` 按 id 从 Chroma 整块取原文注入，注入块标明"已完整注入，无需再调 palace 检索"
- **每个 session 只注入一次**；注过之后再命中触发词就什么都不注（备忘已在上下文里，重复注入只会撑爆 context）
- 未命中触发词的消息照常走原来的模糊检索，日常聊天不受影响
- 网关**每条消息现读**配置文件——改触发词/ids 不用重启网关，但 VPS 上要 `git pull` 拿到 GitHub 上的修改
- `edit_core` 修改备忘内容不换 id，注入的自动是最新版

**链路：** `pinned_memories.json`（触发词+ids）→ 网关 `match_pinned_memories()` → palace `/admin/memories_by_ids`（`claude_memory.claude_get_memories_by_ids`，Chroma 按 id 精确取）→ 注入消息前缀。

日志标记：`Injected pinned memos (3)`。
...
- **亲密备忘固定注入**：触发词命中按 id 整块注入三条描写备忘，跳过模糊检索，每 session 一次
